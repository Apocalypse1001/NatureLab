...
Две доработки 2026-09-01 по итогам проверки в запущенном приложении (числа были правильными, но
эффект не был виден/достаточно быстрым): `_seed_river_profile()` выставляет градиент мгновенно
на переходе flow false→true (не ждёт диффузии от кромок — на старом `FLUID_FLOW_GAIN` середина
сетки не двигалась 30 реальных секунд), и сам `FLUID_FLOW_GAIN` поднят 1.6→10.0 (наибольшее
стабильное значение под worst-case: препятствие + камень + полное охлаждение тенью — см.
`config.py` докстринг и `test_flow_gain_stays_stable_under_worst_case_obstacle_bed_and_shade`).
Плюс чисто визуальный `SceneManager.WATER_VISUAL_EXAGGERATION` на **глубину** (физику не
трогает) — см. `docs/04_TZ_v0.3_roadmap.md` v0.4 за деталями и почему exaggeration должна быть
на depth, а не на абсолютную высоту поверхности (та версия z-fight'ила).
