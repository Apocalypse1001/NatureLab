# NatureLab 0.14.2 — Educational FloodLab / RiverLab / VolcanoLab / TsunamiLab
...
> **Продолжение работы:** [`docs/11_permaculture_plan.md`](docs/11_permaculture_plan.md)
> — почвенная влага, свэйлы и корни; план написан целиком, код не начат. Не закрыты
> также: «лава встречает воду» (вынесено из v0.14.0), отступление моря, ограниченное
> донным трением ([`docs/13_tsunami2_plan.md`](docs/13_tsunami2_plan.md)), и река с
> паводком, отложенная на v0.15.0 — её план и незакрытый дефект солвера в
...
Создаёт `releases\NatureLab_v0.14.2.zip` и записывает его SHA-256 в
...
- Дождя и подъёма уровня во времени нет (осознанно отложено). Источников воды
  теперь несколько: приток с западной кромки, перемещаемый `SOURCE`, `DRAIN`,
  жерло `VENT` и одноразовый посев волны цунами.
