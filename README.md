...
- Дерево не умеет ломаться (DAMAGED/BROKEN, break_strength) и становиться новым препятствием —
  отдельный будущий пункт, требует своей физики разрушения, не часть v0.3 force model. Сейчас
  дерево физически неотличимо от коробки/машины (тот же `ForceRigidBodySystem`, другие
  дефолты) — не закреплено к земле никаким "root_strength", просто сдвигается как только drag
  превышает friction, как любой другой объект.
