...
**Пользователь проверил это в запущенном приложении (2026-09-01) и нашёл, что этого было
недостаточно** — включённый флаг всё равно выглядел как бассейн, поднятый холм выглядел
залитым, вообще никакой динамики не видно. Причина всех трёх одна: при исходном
`FLUID_FLOW_GAIN=1.6` середина сетки 100×100 не сдвигалась со старта **30 реальных секунд**
подряд (измерено). Исправлено: `FLUID_FLOW_GAIN` поднят до 10.0 (наибольшее значение, которое
остаётся стабильным в худшей проверенной комбинации — препятствие + камень + полное охлаждение
тенью одновременно, 90с; нестабильность начинается уже на эффективном gain 16, без запаса), и
`ShallowWaterFluidSolver._seed_river_profile()` выставляет градиент мгновенно при включении
флага вместо ожидания диффузии от кромок. Плюс чисто визуальный множитель
`SceneManager.WATER_VISUAL_EXAGGERATION=4` на глубину (не на физику) — иначе даже правильный
градиент читался на экране как плоский лист. Подробности, включая почему первая попытка
экзажерации дала z-fighting checkerboard, — `docs/04_TZ_v0.3_roadmap.md` v0.4 и
`.claude/skills/run-naturelab/SKILL.md` (Gotchas).
