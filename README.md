...
## River flow: непрерывное течение вместо «устаканенного» озера
...
`world.water.flow_enabled`; при нём каждый шаг западная кромка сетки (`i=0`) удерживается не
ниже `config.FLUID_RIVER_SOURCE_DEPTH`, а восточная (`i=-1`) — не выше
`config.FLUID_RIVER_SINK_DEPTH`. Это исток, который не пересыхает, и устье, которое не
подпирает: между ними всегда есть настоящий перепад высоты, и уже существующая консервативная
flux-схема несёт его как реальное непрерывное течение запад→восток.

- Масса **намеренно** не сохраняется на этих двух кромочных столбцах (вода входит на истоке,
  уходит в устье) — во всей остальной сетке действует ровно та же консервативная схема, что и
  раньше; `test_conservation_no_water_from_nowhere` по-прежнему проверяет её с выключённым flow.
- Читается **живьём каждый tick** (`SimulationManager._step_once` → `set_river_flow`), поэтому,
  в отличие от `water_level`, переключается прямо во время RUNNING.
- Выключено по умолчанию: «озеро» остаётся озером, никакой скрытой смены поведения старых сцен.
- Тесты: `test_flat_water_stays_static_when_flow_disabled` фиксирует сам симптом (документирует
  находку как поведение, а не прячет её), `test_river_flow_creates_sustained_current_on_flat_terrain`
  требует, чтобы течение было **продолжающимся**, а не одноразовым переходным процессом,
  который снова успокоится.
